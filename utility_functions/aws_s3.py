...
def archive_landing_object(filename: str, dataset: str) -> bool:
    try:
        ...
        copy_source = {'Bucket': bucket_name, 'Key': f"s3a://deltalake12/dw-with-pyspark/landing/store/store_20220101.csv"}
        response = client.copy_object(Bucket = bucket_name, CopySource = copy_source, Key = f"s3a://deltalake12/dw-with-pyspark/archive/store/store_20220101.csv")
        if response['ResponseMetadata']['HTTPStatusCode'] == 200:
            client.delete_object(Bucket = bucket_name, Key = f"s3a://deltalake12/dw-with-pyspark/landing/store/store_20220101.csv")
            return True
        else:
            return False
